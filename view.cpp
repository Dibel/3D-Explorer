...
View::View(int width, int height)
    : pickedObject (NULL), enteredObject (NULL), hudObject (NULL), hudEffect (NULL), picture (NULL) {
    resize (width, height);
...
void View::updateDir () {
    ...
    QGLTexture2D* tex = picture->material ()->texture ();
...
static QMatrix4x4 calcMvp (const QGLCamera* camera, const QSize& size) {
    ...
    QMatrix4x4 screenMvp =
        QMatrix4x4 (w / 2, 0, 0, w / 2, 0, -h / 2, 0, h / 2, 0, 0, 1, 0, 0, 0, 0, 1) *
        cameraMvp;
